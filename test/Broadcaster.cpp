...
std::future<void> Broadcaster::OnConnectSendTransport(const json& dtlsParameters)
{
	...
	std::string url = baseUrl + "/broadcasters/" + id + "/transports/"
  ...
  const json& /*appData*/)
{
	...
	httplib::Client cli("localhost", 8888);
	std::string url = baseUrl + "/broadcasters/" + id + "/transports/"
		+ sendTransport->GetId() + "/produce/data";
	auto res = cli.Post(url.c_str(), body.dump(), "application/json");
	if (!res)
	{
		printf("[error] !!!!\n");
	...
	if ( res->status != 200)
	{
		std::cerr << "[ERROR] unable to create Broadcaster"
			<< " [status code:" << res->status << ", body:\"" <<  res->status << "\"]" << std::endl;
...
void Broadcaster::CreateDataConsumer()
{
	...
	httplib::Client cli("localhost", 8888);
	std::string url = baseUrl + "/broadcasters/" +id + "/transports/" +
		recvTransport->GetId() + "/consume/data";
	auto res = cli.Post(url.c_str(), body.dump(), "application/json");
	if (!res)
	{
		printf("[error] !!!!\n");
	...
	this->dataConsumer = recvTransport->ConsumeData(
...
void Broadcaster::CreateSendTransport(bool enableAudio, bool useSimulcast)
{
	...
	json sctpCapabilities = this->device.GetRtpCapabilities();
	/* clang-format off */
	json body =
	{
		{ "type",    "webrtc" },
